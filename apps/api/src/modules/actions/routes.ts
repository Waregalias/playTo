...
import { createActionSchema, actionSchema, actionsResponseSchema } from '@aldenfer/shared';
...
export function registerActionRoutes(app: FastifyInstance, auth: Auth, now: () => Date): void {
  ...
  typed.post(
    ...
    async (request, reply) => {
      const character = await requireCharacter(app.db, auth, request);
      const action = await enqueueAction(app.db, character.id, request.body, now());
  ...
  typed.delete(
    ...
    async (request, reply) => {
      const character = await requireCharacter(app.db, auth, request);
      await cancelAction(app.db, character.id, request.params.id, now());
