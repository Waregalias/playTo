...
| **Grille de bâtiments** (accueil Bastion) | Maquette `WEB_UI_OK_BASTION`. Bannière (image région) avec titre + sous-titre « Dernier feu du royaume » posés dessus. Cartes bâtiment : icône (`buildings/*`) ou fallback initiale Cinzel, nom, description, bouton *Entrer* — verrouillé : bouton cadenas désactivé. Ordre : Forge, Archives, Tableau, Chantre, Beffroi, Hôtel des ventes. 2 colonnes mobile, les 6 sur une rangée en wide.                                                |
| **Écran Héros** (fiche de personnage)     | Maquette `WEB_UI_OK_HERO` / `MOBILE_UI_OK_HERO`. Portrait full-body + identité (nom Cinzel, classe, niveau + jauge XP). Carte **Statistiques** titrée : tuiles compactes FOR/ADR/VOL/VIT/FER + jauges fines PV/Endurance/Charge. Carte **Compétences** sur la fiche : liste (rune, nom, description, tag Passif/Active/Verrouillée) + détail de la sélection avec actions (Apprendre/Équiper/Retirer), bouton « Voir toutes les compétences » vers l'arbre. Rangée basse : **Équipement** (six emplacements nommés Arme/Tête/Armure/Mains/Pieds/Accessoire, `—` si vide) séparé de l'**Inventaire** (grille de cases, sélection → actions) et des **Matériaux** (liste + quantités). Wide : 3 colonnes (portrait / identité+stats / compétences, puis équipement / inventaire / matériaux).                                                |
...
- Couleurs de terrain (fill) — **palette mate atmosphérique** (désaturée + assombrie pour lire sous la brume) : plain `#767E61` · forest `#445541` · hill `#6E6553` · marsh `#4F5952` · ruins `#5A575C` · ash_road `#343B42` · ford `#414F59` · shrine `#897156`. Contour `#0A1119` 1px. Sélection : contour `--color-ember-glow` 2.5px.
- **Voile atmosphérique** : par-dessus la couche des tuiles, un voile sombre translucide uniforme (≈ `rgba(10,15,20,.18)`) unifie la carte dans une ambiance brumeuse et évite que les faces des tuiles ne « ressortent » trop. Marqueur joueur, POI et brume dérivante restent rendus au-dessus du voile.
