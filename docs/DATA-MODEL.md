...
```ts
import {
  pgTable, uuid, varchar, integer, bigint, boolean,
  timestamp, jsonb, text, pgEnum, primaryKey, index, uniqueIndex, check,
...
export const terrainEnum = pgEnum("terrain",
  ["plain", "forest", "hill", "marsh", "ruins", "ash_road", "ford", "shrine"]);
...
export const characters = pgTable("characters", {
  ...
  str: integer("str").notNull(), dex: integer("dex").notNull(),
  wil: integer("wil").notNull(), vit: integer("vit").notNull(),
...
export const inventory = pgTable("inventory", {
  ...
  index("inv_char_idx").on(t.characterId),
...
export const combats = pgTable("combats", {
  ...
  uniqueIndex("combat_active_uq").on(t.characterId).where(sql`status = 'active'`),
]);
...
export const raidAssaults = pgTable("raid_assaults", {
  ...
  characterId: uuid("character_id").notNull().references(() => characters.id),
  combatId: uuid("combat_id").notNull().references(() => combats.id),
...
export const marketListings = pgTable("market_listings", {
  id: uuid("id").defaultRandom().primaryKey(),
  sellerId: uuid("seller_id").notNull().references(() => characters.id),
  itemId: varchar("item_id", { length: 40 }).notNull().references(() => items.id),
  qty: integer("qty").notNull(),
